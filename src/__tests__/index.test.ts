...
test('should return the some function (match)', () => {
  ...
  expect(valueOrNone.match(x => { return x + ' tester'}, () => 'tester')).toBe('test tester');
});

test('should return the none function (match)', () => {
  const valueOrNone = new Optional(undefined);
  expect(valueOrNone.match(x => { return x + ' tester'}, () => 'tester')).toBe('tester');
});


test('should return the some function (matchAsync)', () => {
  const valueOrNone = new Optional('test');
  valueOrNone.matchAsync(async x => { return x + ' tester'}, async () => 'tester').then(data => expect(data).toBe('test tester'));
});

test('should return the none function (matchAsync)', () => {
  const valueOrNone = new Optional(undefined);
  valueOrNone.matchAsync(async x => { return x + ' tester'}, async () => 'tester').then(data => expect(data).toBe('tester'));
});
...
test('should map the value (mapAsync)', () => {
  const valueOrNone = new Optional(1);
  valueOrNone.mapAsync(async a => a*2).then(mappedValue => expect(mappedValue.valueOrFailure()).toBe(2));
});

test('should be mapped to different type (mapAsync)', () => {
  const valueOrNone = new Optional(1);
  valueOrNone.mapAsync(async a => (a*2).toString()).then(mappedValue => expect(mappedValue.valueOrFailure()).toBe("2"));
});

test('should be able to map a None optional (mapAsync)', () => {
  const valueOrNone = new Optional(undefined);
  expect(valueOrNone.hasValue).toBe(false);
  valueOrNone.mapAsync(async a => a*2).then(mappedOrNone => expect(mappedOrNone.hasValue).toBe(false));
});
...
test('should be flatmapped to different type (flatMap)', () => {
  const valueOrNone = new Optional(1);
  expect(valueOrNone.flatMap(a => Some((a*2).toString())).valueOrFailure()).toBe("2");
});
...
test('should flatmap the value (flatMapAsync)', () => {
  const valueOrNone = new Optional(1);
  valueOrNone.flatMapAsync(async a => Some(a*2)).then(mappedValue => expect(mappedValue.valueOrFailure()).toBe(2));
});

test('should be flatmapped to different type (flatMapAsync)', () => {
  const valueOrNone = new Optional(1);
  valueOrNone.flatMapAsync(async a => Some((a*2).toString())).then(mappedValue => expect(mappedValue.valueOrFailure()).toBe("2"));
});

test('should be able to flatmap a None optional (flatMapAsync)', () => {
  const valueOrNone = new Optional(undefined);
  expect(valueOrNone.hasValue).toBe(false);
  valueOrNone.flatMapAsync(async a => Some(a*2)).then(mappedOrNone => expect(mappedOrNone.hasValue).toBe(false));
});
...
test('should fail with an error (valueOrFailure)', () => {
  const valueOrNone = new Optional(undefined);
  expect(() => { valueOrNone.valueOrFailure()}).toThrowError('There exists no value');
});

test('should fail with specified error message (valueOrFailure)', () => {
  const valueOrNone = new Optional(undefined);
  expect(() => { valueOrNone.valueOrFailure('custom message')}).toThrowError('custom message');
});
...
test('should return error (valueOr)', () => {
  const valueOrNone = new Optional(undefined);
  expect(() =>  { valueOrNone.valueOr(new Error('test'))}).toThrowError('test');

});

test('should return error function (valueOr)', () => {
  const valueOrNone = new Optional(undefined);
  expect(() => valueOrNone.valueOr(() => {throw new Error('test')})).toThrowError('test');
});

test('should return the value (valueOrAsync)', () => {
  const valueOrNone = new Optional('test');
  valueOrNone.valueOrAsync(new Promise((resolve) => resolve('Different'))).then(val => expect(val).toBe('test'));
})


test('should return alternative value (valueOrAsync)', () => {
  const valueOrNone = new Optional(undefined);
  valueOrNone.valueOrAsync(new Promise((resolve) => resolve('different'))).then(val => expect(val).toBe('different'));
});

test('should return error (valueOrAsync)', () => {
  const valueOrNone = new Optional(undefined);
  expect(() =>  { valueOrNone.valueOrAsync(new Error('test'))}).toThrowError('test');

});

test('should return error function (valueOrAsync)', () => {
  const valueOrNone = new Optional(undefined);
  expect(() => valueOrNone.valueOrAsync(() => {throw new Error('test')})).toThrowError('test');
});
...
test('should convert advanced objects to JSON', () => {
  ...
  expect(JSON.stringify(advancedObjectOrNone)).toBe(JSON.stringify(advancedObject));
});
...
test('should convert and return a list (ConvertAndGetValuesAsync)', () => {
  const optionalList = [1,2,3];
  ConvertAndGetValuesAsync(optionalList, async val => val.map(a => a + '')).then(list => expect(list).toEqual(['1','2','3']));
});
